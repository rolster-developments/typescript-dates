...
describe('formatDate', () => {
  it('should format for date template successful', () => {
    expect(dateFormatTemplate(date, '{dd}, {mx} del {aa}')).toBe(
      '30, May del 2024'
    );
  ...
  it('should format for datetime template successful', () => {
    expect(dateFormatTemplate(date, '{aa}-{mm}-{dd} {hh}:{ii}:{ss}')).toBe(
      '2024-05-30 10:04:24'
    );
    expect(dateFormatTemplate(date, '{mx}. {dd}, {aa} {hz}:{ii} {zz}')).toBe(
      'May. 30, 2024 10:04 AM'
    );
    expect(dateFormatTemplate(date, '{aa}-{mm}-{dd}T{hh}:{ii}:{ss}')).toBe(
      '2024-05-30T10:04:24'
    );
  });
});
